...
__global__ void wmma_example(float *a, float *b, float *c, int M, int N, int K, unsigned long long iterations) {
...
//   wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, float, wmma::precision::tf32> a_frag;
//   wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, float, wmma::precision::tf32> b_frag;
   ...
   wmma::load_matrix_sync(c_frag, c , ldc, wmma::mem_col_major);
    ...
    for (unsigned long long i = 0; i < iterations; i++){
        // Inline PTX assembly for the mma instruction.
        // We use the m16n16k8.col.col variant to match the fragment dimensions and layouts.
        // The .f32.tf32.tf32.f32 suffix specifies the data types of D, A, B, and C respectively.
        asm volatile(
...
//   convertFp32ToFp16 <<< (MATRIX_K * MATRIX_N + 255) / 256, 256 >>> (b_fp16, b_fp32, MATRIX_K * MATRIX_N);
   ...
   wmma_example <<< gridDim, blockDim >>> (a_fp32, b_fp32, c_wmma, MATRIX_M, MATRIX_N, MATRIX_K, iterations);
